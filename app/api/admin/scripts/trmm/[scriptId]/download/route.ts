...
import { downloadTrmmScript } from '@/lib/trmm/scripts';

export const dynamic = 'force-dynamic';

type ScriptDownloadRouteContext = {
  params: Promise<{
    scriptId: string;
  }>;
};
...
export async function GET(
  request: NextRequest,
  context: ScriptDownloadRouteContext,
) {
  try {
    ...
    if (!Number.isFinite(numericScriptId) || numericScriptId <= 0) {
      return NextResponse.json(
        {
          ok: false,
          error: 'ID do script inválido.',
        },
        { status: 400 },
      );
    }

    const script = await downloadTrmmScript(numericScriptId);
